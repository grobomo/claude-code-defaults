 ...
 *   - dependency checking (is rule-manager installed?)
 *   - rule file installation (via rule-manager or fallback)
 ...
 * @param {string} managerName - e.g. 'hook-manager', 'rule-manager'
...
function backup(managerName, filePaths) {
  ...
  for (var i = 0; i < filePaths.length; i++) {
    ...
    // Preserve subdirectory structure for rule files
    if (src.indexOf(path.join('rules', 'UserPromptSubmit')) !== -1) {
      relName = path.join('rules', 'UserPromptSubmit', path.basename(src));
      fs.mkdirSync(path.join(backupDir, 'rules', 'UserPromptSubmit'), { recursive: true });
    } else if (src.indexOf(path.join('rules', 'Stop')) !== -1) {
      relName = path.join('rules', 'Stop', path.basename(src));
      fs.mkdirSync(path.join(backupDir, 'rules', 'Stop'), { recursive: true });
    }
...
/**
 * Install a rule file. Uses rule-manager if available,
 * otherwise writes directly to ~/.claude/rules/<event>/.
 *
 * @param {object} opts
 ...
 */
function installRule(opts) {
  var event = opts.event || 'UserPromptSubmit';
  var destDir = path.join(RULES_DIR, event);
  var destPath = path.join(destDir, opts.id + '.md');

  // Always write to disk (rule-manager reads from same location)
  ...
  var imInstalled = checkDependency('rule-manager').installed;
  return {
    method: imInstalled ? 'rule-manager' : 'direct-write',
    path: destPath,
...
/**
 * Remove a rule file (archive, never delete).
 * @param {string} id - rule id
 * @param {string} event - 'UserPromptSubmit' or 'Stop'
 * @returns {string|null} archive path, or null if not found
 */
function removeRule(id, event) {
  event = event || 'UserPromptSubmit';
  var srcPath = path.join(RULES_DIR, event, id + '.md');
  if (!fs.existsSync(srcPath)) return null;

  var archiveDir = path.join(RULES_DIR, 'archive');
  fs.mkdirSync(archiveDir, { recursive: true });
  var ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
...
// -------------------------------------------------------------------------
// Routing rule template (auto-create during skill/MCP setup)
// -------------------------------------------------------------------------

/**
 * Generate and install a routing rule that teaches Claude WHEN to use
 * a specific skill or MCP server instead of generic tools (WebFetch, Bash, etc).
 *
 * Call this from any skill or MCP setup script. It uses installRule()
 * internally, with fallback to direct file write if rule-manager
 * is not installed.
 *
 * @param {object} opts
 * @param {string} opts.toolName    - skill or MCP name, e.g. 'wiki-api'
 * @param {string} opts.toolType    - 'skill' or 'mcp'
 * @param {string[]} opts.keywords  - trigger keywords for this rule
 ...
 * @returns {{ method: string, path: string, fallback: boolean }}
 */
function ensureRoutingRule(opts) {
  ...
  return installRule({ id: id, content: body, event: 'UserPromptSubmit' });
}
...
// -------------------------------------------------------------------------
// Global MCP registration (.claude.json mcpServers)
// -------------------------------------------------------------------------

/**
 * Register an MCP server globally in .claude.json mcpServers.
 * This makes the server available in ALL projects, not just the current directory.
 * .claude.json is the user-level config that Claude Code reads at session start.
 *
 * @param {string} name - server name (e.g. 'mcp-manager')
 * @param {object} config - server config (command, args, env, or type+url for remote)
 * @returns {{ method: string, registered: boolean, path: string }}
 */
function ensureGlobalMcp(name, config) {
  var claudeJson = path.join(HOME, '.claude.json');
  var data;
  try {
  ...
  var existing = data.mcpServers[name];
  if (existing) {
    var sameCommand = existing.command === config.command;
    var sameUrl = existing.url === config.url;
    var sameArgs = JSON.stringify(existing.args) === JSON.stringify(config.args);
    if ((sameCommand && sameArgs) || sameUrl) {
      return { method: 'skipped', registered: true, path: claudeJson };
    }
  }

  data.mcpServers[name] = config;
  fs.writeFileSync(claudeJson, JSON.stringify(data, null, 2), 'utf8');
  return { method: 'registered', registered: true, path: claudeJson };
}

/**
 * Remove an MCP server from .claude.json mcpServers.
 * @param {string} name - server name
 * @returns {boolean} true if removed
 */
function removeGlobalMcp(name) {
  var claudeJson = path.join(HOME, '.claude.json');
  var data;
  try {
    data = JSON.parse(fs.readFileSync(claudeJson, 'utf8'));
  ...
  if (!data.mcpServers || !data.mcpServers[name]) return false;
 ...
 * @param {object[]} opts.rules - rule install results
...
function printSummary(opts) {
  ...
  if (opts.rules && opts.rules.length > 0) {
    console.log('[' + opts.manager + ':setup] Rules:');
    for (var i = 0; i < opts.rules.length; i++) {
      var inst = opts.rules[i];
...
module.exports = {
  ...
  ensureRoutingRule: ensureRoutingRule,
  ...
  ensureGlobalMcp: ensureGlobalMcp,
  removeGlobalMcp: removeGlobalMcp,
  ...
  installInstruction: installRule,
  ensureRoutingInstruction: ensureRoutingRule,
  removeInstruction: removeRule
};
